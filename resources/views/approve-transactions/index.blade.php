...
@push('scripts')
    <script>
        ...
        function approve(identifier) {
            Swal.fire({
                title: "Apakah anda ingin mengapprove data ini?",
                icon: "warning",
                ...
                if (result.isConfirmed) {
                    ...
                    let url = '{{ route('approve-transactions.approve', ':id') }}'
                    url = url.replace(':id', identifier.getAttribute('data-id'))
                    $.ajax({
                        url: url,
                        type: 'PUT',
                        ...
                        success: function(result) {
                            Swal.fire({
                                title: "Sukses!",
                                text: "Data berhasil di approved.",
                                icon: "success"
                            }).then((result) => {
                                if (result.isConfirmed) {
                                    location.reload();
        ...
        function detail(identifier) {
            id = identifier.getAttribute('data-id')
            let urlShow = '{{ route('approve-transactions.show', ':id') }}'
            ...
            $.ajax({
                ...
                success: function(result) {
                    if (result.success) {
                        ...
                        if (data.outlet_name == data.transactions.outlet_name) {

                            $('#modal-detail #outlet_name').html(data.outlet_name)
                        } else {
                            $('#modal-detail #outlet_name').html('Sebelum: ' + data.transactions.outlet_name +
                                '<br> <br>' + 'Sesudah: ' + data.outlet_name + '<br> <br>' +
                                'Alasan Perubahan Data Outlet: ' + data.reason);
                        }
                        if (data.transactions.residency_name == data.residency_name) {
                            $('#modal-detail #residency_name').html(data.residency_name)
                        } else {
                            $('#modal-detail #residency_name').html('Sebelum: ' + data.transactions
                                .residency_name +
                                '<br> <br>' + 'Sesudah: ' + data.residency_name);
                        }

                        if (data.transactions.city_name == data.city_name) {
                            $('#modal-detail #city_name').html(data.city_name)
                        } else {
                            $('#modal-detail #city_name').html('Sebelum: ' + data.transactions
                                .city_name +
                                '<br> <br>' + 'Sesudah: ' + data.city_name);
                        }

                        if (data.transactions.district_name == data.district_name) {
                            $('#modal-detail #district_name').html(data.district_name)
                        } else {
                            $('#modal-detail #district_name').html('Sebelum: ' + data.transactions
                                .district_name +
                                '<br> <br>' + 'Sesudah: ' + data.district_name);
                        }
                        ...
                        $('#product_list').html(products(data.details, data.transactions.details))

                        $('#modal-detail #description').val(description(data.details, data.transactions
                            .details))
                        $('#modal-detail #additional_description').val(data.additional_description)
                        $('#modal-detail #reason').val(data.reason)
        ...
        const description = (data, detail) => {
            ...
            let html = ''
            for (let i = 0; i < data.length; i++) {
                arr.push(data[i].product_code);

                html += `${data[i].qty} ${data[i].unit} ${data[i].product_name}, `
            }

            for (let i = 0; i < detail.length; i++) {
                if (arr.includes(detail[i].product_code)) {
                    continue;
                }
                html +=
                    `${detail[i].qty} ${detail[i].unit} ${detail[i].product_name}, `
            }
        ...
        const products = (data, detail) => {
            ...
            for (let i = 0; i < data.length; i++) {
                ...
                if (data[i].reason_type == 'EDIT' || data[i].reason_type == null) {
                    arr.push(data[i].product_code);
                    html +=
                        `<li>${data[i].product_name} (${data[i].product_code})  </li>`
                    html += `- Sebelum: ${data[i].detail_transaction.qty} ${data[i].detail_transaction.unit} <br>`
                    html += `- Sesudah: ${data[i].qty} ${data[i].unit} <br>`
                    html += `- Alasan perubahan data:  ${data[i].reason}`
                    html += `<br><br>`
                }

                if (data[i].reason_type == 'DELETE') {
                    arr.push(data[i].product_code);
                    html +=
                        `<li>${data[i].product_name} (${data[i].product_code}) (${data[i].qty}) (${data[i].unit})  </li>`
                    html += `- Alasan penghapusan data:  ${data[i].reason}`
                    html += `<br><br>`
            ...
            for (let i = 0; i < detail.length; i++) {
                ...
                html +=
                    `<li>${detail[i].product_name} (${detail[i].product_code}) (${detail[i].qty} ${detail[i].unit})  </li>`
            }
