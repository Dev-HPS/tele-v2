...
@section('content')
    <div class="row align-items-center">
        <div class="col-md-2">
            <div class="mb-3">
                <a href="{{ route('inquiry.create') }}" class="btn btn-primary"><i class="bx bx-plus me-1"></i> Tambah</a>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function status(identifier, param) {
            ...
            Swal.fire({
                ...
                if (e.value) {
                    url = '{{ route('inquiry.update-status', ':inquiry') }}'
                    url = url.replace(':inquiry', identifier.getAttribute('data-id'))
                    $.ajax({
                        url: url,
                        type: 'PUT',
                        cache: false,
